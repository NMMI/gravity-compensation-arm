...
void get_param_list(uint16 index) {
    ...
    char abs_enc_str[36] = "14 - Absolute encoder position:";
    char handle_ratio_str[25] = "15 - Motor handle ratio:"; 
    char motor_type_str[24] = "16 - PWM rescaling:";
    char curr_lookup_str[21] = "17 - Current lookup:";
    char closure_speed_str[20]  = "18 - Closure speed:";
    char joystick_thr_str[26]   = "19 - Joystick thresholds:";
    char joystick_gains_str[21] = "20 - Joystick gains:";

    //Parameters menus
    char input_mode_menu[39] = "0 -> Usb\n1 -> Handle\n2 -> Joystick\n";
    ...
    switch(index) {
        case 0:         //List of all parameters with relative types
            ...
            *((int16 *)(packet_data + 604)) = c_mem.current_limit;
            ...
            packet_data[653] = 1;
            packet_data[654] = c_mem.double_encoder_on_off;
            ...
            for(i = string_lenght; i != 0; i--)
                packet_data[655 + string_lenght - i] = abs_enc_str[string_lenght - i];
            //The following byte indicates the number of menus at the end of the packet to send
            ...
            *((int8 *)(packet_data + 704)) = c_mem.motor_handle_ratio;
            ...
            packet_data[802] = TYPE_FLOAT;
            packet_data[803] = 6;
            for(i = 0; i < LOOKUP_DIM; i++)
                *((float *) ( packet_data + 804 + (i * 4) )) = c_mem.curr_lookup[i];
            for(i = curr_lookup_str_len; i != 0; i--)
                packet_data[828 + curr_lookup_str_len - i] = curr_lookup_str[curr_lookup_str_len - i];
            
            /*-------------CLOSURE SPEED------------*/

            packet_data[852] = TYPE_UINT16;
            packet_data[853] = 1;
            *((uint16 *) (packet_data + 854)) = c_mem.closure_speed;
            for(i = closure_speed_str_len; i != 0; i--)
                packet_data[856 + closure_speed_str_len - i] = closure_speed_str[closure_speed_str_len - i]; 

            /*----------JOYSTICK THRESHOLD----------*/

            packet_data[902] = TYPE_INT16;
            packet_data[903] = 1;
            *((int16 *) (packet_data + 904)) = c_mem.joystick_threshold;
            for(i = joystick_thr_str_len; i != 0; i--)
                packet_data[906 + joystick_thr_str_len - i] = joystick_thr_str[joystick_thr_str_len - i];

            /*------------JOYSTICK GAINS------------*/

            packet_data[952] = TYPE_UINT16;
            packet_data[953] = 2;
            *((uint16 *) packet_data + 954) = c_mem.joystick_gains[0];
            *((uint16 *) packet_data + 956) = c_mem.joystick_gains[1];
            for(i = joystick_gains_str_len; i != 0; i--)
                packet_data[958 + joystick_gains_str_len - i] = joystick_gains_str[joystick_gains_str_len - i];
            ...
            for(i = yes_no_menu_len; i!= 0; i--)
                packet_data[1252 + yes_no_menu_len - i] = yes_no_menu[yes_no_menu_len - i];
...
//================================================     set_double_encoder_on_off
        case 14:        //Absolute encoder flag - uint8
...
//===================================================     set_curr_lookup_table
        case 17:        //Current lookup table - float
            for(i = 0; i < LOOKUP_DIM; i++)
...
void infoPrepare(unsigned char *info_string)
{
    ...
    strcat(info_string,"\r\n");
        ...
        sprintf(str, "Joystick measurements: %hd, %hd", g_meas.joystick[0], g_meas.joystick[1]);
        strcat(info_string, str);
        strcat(info_string, "\r\n");
